...
from trainer1 import BaseNetwork
...
class Network(BaseNetwork):
    ...
    def __init__(self, train_list_raw, test_list_raw, png_folder, batch_size, dropout, l2, mode, batch_norm,
                 rnn_num_units, **kwargs):
        ...
        # CONV-RELU-POOL 1
        network = layers.Conv2DLayer(incoming=network, num_filters=16, filter_size=(7, 7), stride=1, nonlinearity=rectify)
        ...
        # CONV-RELU-POOL 2
        network = layers.Conv2DLayer(incoming=network, num_filters=32, filter_size=(5, 5), stride=1, nonlinearity=rectify)
        network = layers.MaxPool2DLayer(incoming=network, pool_size=(3, 3), stride=2, pad=2)
        if self.batch_norm:
            network = layers.BatchNormLayer(incoming=network)

        # CONV-RELU-POOL 3
        network = layers.Conv2DLayer(incoming=network, num_filters=32, filter_size=(3, 3), stride=1, nonlinearity=rectify)
        network = layers.MaxPool2DLayer(incoming=network, pool_size=(3, 3), stride=2, pad=2)
        if (self.batch_norm):
            network = layers.BatchNormLayer(incoming=network)
        ...
        for channel_index in range(num_channels):
            ...
            if channel_index == 0:
                ...
                # GRULayer, but shared
                network = layers.GRULayer(incoming=network, num_units=self.num_units, only_return_final=True,
                                          resetgate=layers.Gate(W_in=W_in_to_resetgate, W_hid=W_hid_to_resetgate,
                                                                b=b_resetgate),
                                          updategate=layers.Gate(W_in=W_in_to_updategate, W_hid=W_hid_to_updategate,
                                                                 b=b_updategate),
                                          hidden_update=layers.Gate(W_in=W_in_to_hidden_update,
                                                                    W_hid=W_hid_to_hidden_update, b=b_hidden_update))
